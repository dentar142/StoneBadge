 ...
 */
export async function generateTemplate(outputPath) {
  ...
  // 选择模型来源：env MODEL=toilet 走程序化马桶，否则走原 GLB 石墩子
  const useToilet = (process.env.MODEL || '').toLowerCase() === 'toilet';

  return new Promise(async (resolve, reject) => {
    try {
      let model;
      if (useToilet) {
        const { buildToilet } = await import('./toilet-geometry.js');
        model = buildToilet(THREE);
        console.log('使用程序化马桶几何体（MODEL=toilet）');
      } else {
        ...
        model = await new Promise((res, rej) => {
          new GLTFLoader().parse(ab, '', (gltf) => res(gltf.scene),
            (err) => rej(new Error(`GLB 模型解析失败: ${err?.message || err}`)));
        });
      }
      {
        // 兼容原有处理流程：原代码假设 gltf 回调里直接拿到 model
        // 此处不再嵌套额外的回调，直接顺序执行
        ...
        for (let frame = 0; frame < FRAME_COUNT; frame++) {
          ...
          // 面填充路径（stroke 仅用于防 0.2px 亚像素缝隙，不可见）
          let paths = '';
          for (const f of faces) {
            const gray = Math.round(f.lum * 204);
            const d = `M${ri(f.v0.x)},${ri(f.v0.y)}L${ri(f.v1.x)},${ri(f.v1.y)}L${ri(f.v2.x)},${ri(f.v2.y)}z`;
            paths += `<path d="${d}" fill="rgb(${gray},${gray},${gray})" stroke="rgb(${gray},${gray},${gray})"/>`;
          }
          ...
          allFramesSvg +=
            ...
            `dur="${ANIM_DURATION}s" repeatCount="indefinite"/>` +
            paths + `</g>`;
